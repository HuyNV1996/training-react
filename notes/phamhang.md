...
| **API Routes**| | Cho phép tạo các API endpoints trực tiếp trong thư mục pages/api, giúp bạn xây dựng backend APIs trong cùng một dự án mà không cần một server riêng biệt.
Hỗ trợ xuất các trang web tĩnh, giúp bạn dễ dàng triển khai các trang tĩnh mà không cần server. | Không hỗ trợ tính năng API routes. Bạn cần thiết lập một server backend riêng biệt nếu muốn cung cấp API.
Không hỗ trợ xuất các trang tĩnh một cách trực tiếp, tất cả nội dung phải được tải từ server. |
...
4. **Khi nào nên sử dụng Next.js?**
    - Cần phải dựa vào tính chất của dự án để quyết định xem có nên sử dụng NextJS hay không. Nếu như Create React App là lựa chọn tốt nếu muốn phát triển ứng dụng React _đơn giản và nhanh chóng_, trong khi Next.js là lựa chọn tốt nếu muốn phát triển ứng dụng React _phức tạp hơn với tính năng bổ sung như SSR_.

5. **Cách cài đặt Next.js?**

6. **Thư mục `pages` trong Next.js có ý nghĩa gì?**
    - Thư mục `Pages`: Chứa các page của trang web, tên folder trong thư mục pages sẽ là route cho các page. Và trong thư mục pages cũng có sẵn thư mục api: đây là phần để viết API xử lý backend cho hệ thống.
...
8. **Làm thế nào để bắt đầu một dự án Next.js?**
    ...
    - Nếu bạn muốn sử dụng TypeScript, bạn có thể thêm TypeScript vào dự án Next.js bằng cách chạy `npm install --save-dev typescript @types/react @types/node`. Sau đó, đổi tên tệp `.js` thành `.tsx` và chạy lệnh `npm run dev`. Next.js sẽ tự động tạo ra tệp `tsconfig.json` cho bạn.
    - Khi bạn đã sẵn sàng triển khai ứng dụng, bạn có thể chạy lệnh sau để xây dựng ứng dụng `npm run build`
    - Sau đó, bạn có thể chạy ứng dụng ở chế độ sản xuất bằng lệnh `npm start` 
...
9. **Cách chạy ứng dụng Next.js trong chế độ phát triển?**
    ...
    - Nếu bạn vừa tạo dự án hoặc tải về một dự án có sẵn, bạn cần cài đặt các phụ thuộc cần thiết. Chạy lệnh sau trong terminal `npm install`
...
| **Nhược điểm**| - Website không linh động, nội dung sẽ trở nên lỗi thời nếu thay đổi quá thường xuyên vì là web tĩnh (có thể dùng Ajax để xử lý dữ liệu động nhưng nó sẽ không được cache cũng như không thân thiện SEO)
- Khả năng mở rộng không tốt vì mỗi lần cập nhật dữ liệu là phải qua quá trình build tốn khá nhiều thời gian.
- Thời gian build tăng lên dựa vào size của project| Không thể deploy đến một static hosting.
